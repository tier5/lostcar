var hljs=new function(){function k(v){return v.replace(/&/gm,"&amp;").replace(/</gm,"&lt;").replace(/>/gm,"&gt;")}
...
function u(x){var v=[];(function w(y,z){for(var A=y.firstChild;A;A=A.nextSibling)if(A.nodeType==3){z+=A.nodeValue.length}else if(t(A)=="br"){z+=1}else if(A.nodeType==1){v.push({event:"start",offset:z,node:A});z=w(A,z);v.push({event:"stop",offset:z,node:A})};return z})(x,0);return v}
function q(w,y,C){var x=0,F="",z=[]
...
function A(H){function G(I){return" "+I.nodeName+'="'+k(I.value)+'"'};F+="<"+t(H)+Array.prototype.map.call(H.attributes,G).join("")+">"}
function E(G){F+="</"+t(G)+">"}
function v(G){(G.event=="start"?A:E)(G.node)};while(w.length||y.length){var D=B();F+=k(C.substr(x,D[0].offset-x));x=D[0].offset;if(D==w){z.reverse().forEach(E);do{v(D.splice(0,1)[0]);D=B()}while(D==w&&D.length&&D[0].offset==x);z.reverse().forEach(A)}else{if(D[0].event=="start"){z.push(D[0].node)}else z.pop();v(D.splice(0,1)[0])}};return F+k(C.substr(x))}
function m(y){function v(z){return(z&&z.source)||z}
function w(A,z){return RegExp(v(A),"m"+(y.cI?"i":"")+(z?"g":""))}
function x(D,C){if(D.compiled)return;D.compiled=true;D.k=D.k||D.bK;if(D.k){var z={};function E(G,F){if(y.cI)F=F.toLowerCase();F.split(" ").forEach(function(H){var I=H.split("|");z[I[0]]=[G,I[1]?Number(I[1]):1]})};if(typeof D.k=="string"){E("keyword",D.k)}else Object.keys(D.k).forEach(function(F){E(F,D.k[F])});D.k=z};D.lR=w(D.l||/\b[A-Za-z0-9_]+\b/,true);if(C){if(D.bK)D.b=D.bK.split(" ").join("|");if(!D.b)D.b=/\B|\b/;D.bR=w(D.b);if(!D.e&&!D.eW)D.e=/\B|\b/;if(D.e)D.eR=w(D.e);D.tE=v(D.e)||"";if(D.eW&&C.tE)D.tE+=(D.e?"|":"")+C.tE};if(D.i)D.iR=w(D.i);if(D.r===undefined)D.r=1;if(!D.c)D.c=[];var B=[];D.c.forEach(function(F){if(F.v){F.v.forEach(function(G){B.push(o(F,G))})}else B.push(F=="self"?D:F)});D.c=B;D.c.forEach(function(F){x(F,D)});if(D.starts)x(D.starts,C);var A=D.c.map(function(F){return F.bK?"\\.?\\b("+F.b+")\\b\\.?":F.b}).concat([D.tE]).concat([D.i]).map(v).filter(Boolean);D.t=A.length?w(A.join("|"),true):{exec:function(F){return null}};D.continuation={}};x(y)}
...
function N(){var U=k(C);if(!I.k)return U;var T="",X=0;I.lR.lastIndex=0;var V=I.lR.exec(U);while(V){T+=U.substr(X,V.index-X);var W=E(I,V);if(W){H+=W[1];T+=w(W[0],V[0])}else T+=V[0];X=I.lR.lastIndex;V=I.lR.exec(U)};return T+U.substr(X)}
