...
function ValueRangeWidget() {
  jQuery('div.value-range').not('.assigned').each(
    function() {
      ...
      var unit = min_value.attr('data-unit');
      var min_default = parseFloat(min_value.attr('data-price'));
      var max_value = jQuery(this).find('.max-value');
      var max_default = parseFloat(max_value.attr('data-price'));
      var select = jQuery(this);
      select.addClass('assigned');
      // var slider = jQuery('<div></div>').appendTo(select).slider({
      var slider = jQuery('<div></div>').insertBefore(min_value).slider({
        range: true,
